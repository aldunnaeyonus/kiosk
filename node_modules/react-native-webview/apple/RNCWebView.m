...
- (void)visitSource
{
  ...
  [self syncCookiesToWebView:^{
    ...
    // the existing page.
	  if ([request.URL isEqual:self->_webView.URL]) {
      return;
    }
    if (!request.URL) {
      // Clear the webview
    ...
    if (request.URL.host) {
		...
		NSURL* readAccessUrl = self->_allowingReadAccessToURL ? [RCTConvert NSURL:self->_allowingReadAccessToURL] : request.URL;
		[self->_webView loadFileURL:request.URL allowingReadAccessToURL:readAccessUrl];
    }
  }];
}
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
      ...
      for (NSHTTPCookie *cookie in cookies) {
        dispatch_group_enter(group);
		  [self->_webView.configuration.websiteDataStore.httpCookieStore setCookie:cookie completionHandler:^{
